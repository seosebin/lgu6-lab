        ...
        with conn.cursor() as cur:
            ...
            cur.execute("""
                CREATE TABLE IF NOT EXISTS airflow_highway_traffic (
                    ...
                    updowntypecode VARCHAR(1),
                    vdsid VARCHAR(50),
                    trafficamout INTEGER,
                    shareratio FLOAT,
                    conzoneid VARCHAR(50),
                    conzonename VARCHAR(100),
                    speed INTEGER,
                    timeavg INTEGER,
                    grade INTEGER,
            ...
            """)
            conn.commit()
        print("PostgreSQL 테이블 생성/확인 완료")
    except Exception as e:
        print(f"PostgreSQL 테이블 생성 중 오류 발생: {str(e)}")
        raise
...
def process_dataframe(**context):
    ...
    try:
        ...
        # 데이터 타입 변환
        numeric_columns = ['trafficamout', 'speed', 'shareratio', 'timeavg', 'grade']
...
def get_mysql_connection():
    ...
    try:
        ...
        print(f"MySQL 연결 중 오류 발생: {str(e)}")
        raise

def create_mysql_table_if_not_exists(conn):
    """MySQL 테이블이 없으면 생성하는 함수"""
    try:
        with conn.cursor() as cur:
            # 테이블이 없을 경우에만 생성
            cur.execute("""
        ...
        print(f"MySQL 테이블 생성 중 오류 발생: {str(e)}")
        raise

def load_to_mysql(**context):
    """MySQL에 데이터를 로드하는 함수"""
    try:
        print("MySQL 데이터 로드 시작")
        ...
        # 테이블 생성 확인
        create_mysql_table_if_not_exists(conn)
        ...
        with conn.cursor() as cur:
            ...
            # XCom에 통계 정보 저장
            context['task_instance'].xcom_push(key='mysql_database_stats', value={
        ...
        print(f"\nMySQL에 데이터 로드 완료: {len(new_records)}개 레코드")
        return f"{len(new_records)}개의 레코드가 MySQL에 추가되었습니다. 현재 총 레코드는 {total_records}개 입니다."
        
    except Exception as e:
        print(f"MySQL 데이터 로드 중 오류 발생: {str(e)}")
        raise
    finally:
        if conn:
            conn.close()
...
def load_to_postgres(**context):
    ...
    try:
        ...
        return f"{len(new_records)}개의 레코드가 PostgreSQL에 추가되었습니다. 현재 총 레코드는 {total_records}개 입니다."
...
def print_data_summary(**context):
    ...
    try:
        ...
        mysql_stats = context['task_instance'].xcom_pull(task_ids='load_to_mysql', key='mysql_database_stats')
        ...
        print(f"4. MySQL 데이터베이스 통계:")
        print(f"   - 전체 레코드 수: {mysql_stats['total_records']}")
        print(f"   - 고유 수집 시간 수: {mysql_stats['unique_times']}")
        print(f"   - 가장 오래된 데이터: {mysql_stats['oldest_record']}")
        print(f"   - 최신 데이터: {mysql_stats['newest_record']}")
        print(f"   - 새로 추가된 레코드 수: {mysql_stats['new_records_added']}")
...
# DAG 정의
dag = DAG(
    'step10_highway_load_mysql_postgresql',
    default_args=default_args,
    description='고속도로 교통량 데이터 수집 및 처리 DAG',
    schedule_interval='*/1 * * * *',  # 1분마다 실행
    catchup=False,
    tags=['highway', 'traffic', 'mysql', 'postgres']
)
...
# 작업 순서 정의
download_task >> process_task >> [load_postgres_task, load_mysql_task] >> summary_task
