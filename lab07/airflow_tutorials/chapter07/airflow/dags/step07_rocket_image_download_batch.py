...
def download_json_data():
    ...
    try:
        ...
        # 현재 시간 기준으로 최근 24시간 내의 발사 데이터를 가져옴
        current_time = datetime.utcnow()
        start_time = current_time - timedelta(hours=24)
        end_time = current_time + timedelta(hours=24)
        
        # API URL에 시간 범위 추가
        api_url = f"https://ll.thespacedevs.com/2.2.0/launch/upcoming/?limit=5&net__gte={start_time.isoformat()}&net__lte={end_time.isoformat()}"
...
# DAG 정의
dag = DAG(
    'step07_rocket_image_download_batch',
    default_args=default_args,
    description='로켓 발사 이미지 다운로드 (5분 간격)',
    schedule_interval='*/5 * * * *',  # 5분마다 실행
    catchup=False,
    tags=['rocket', 'images', 'download', 'batch']
)
